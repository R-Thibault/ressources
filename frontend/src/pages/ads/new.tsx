...
export default function NewAd(props: LayoutProps): React.ReactNode {
  ...
  async function updateData(item: DataEntries) {
    try {
      const result = await axios.put(`${API_URL}/ads/${query}`, item);
      if (result.status === 200) {
        router.push("/");
      }
    } catch (error) {
  ...
  return (
    <Layout title={"Poster une annonce"}>
      ...
      <form onSubmit={(e) => submit(e)} className={styles.form}>
        ...
        <button className={styles.button} type="submit">
          {query ? "Modifier l'annonce" : "Poster l'annonce"}
        </button>
      </form>
    </Layout>
  );
